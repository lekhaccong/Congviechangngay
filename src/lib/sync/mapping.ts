...
export async function toCloud(entityType: SyncEntityType, value: unknown): Promise<Record<string, unknown>> {
  ...
  return { id: row.id, employee_id: row.employeeId, work_date: row.date, manager_shift_id: row.shiftId, actual_shift_code: row.actualShiftCode ?? null, status: row.status, note: row.note, late_minutes: row.lateMinutes ?? 0, confirmed_at: row.confirmedAt ? new Date(row.confirmedAt).toISOString() : null, confirmed_by_name: row.confirmedBy ?? null, check_in: row.checkIn ? new Date(row.checkIn).toISOString() : null, check_out: row.checkOut ? new Date(row.checkOut).toISOString() : null, ot_minutes: row.otMinutes, client_created_at: new Date(row.createdAt).toISOString(), deleted_at: null };
}
...
export async function applyCloudRow(entityType: SyncEntityType, row: Record<string, any>): Promise<void> {
  ...
  if (entityType === "employees") {
    ...
    await db.attendance.put({ id: row.id, employeeId: row.employee_id, date: row.work_date, shiftId: row.manager_shift_id, actualShiftCode: row.actual_shift_code ?? undefined, status: row.status, note: row.note ?? "", lateMinutes: row.late_minutes ?? 0, confirmedAt: millis(row.confirmed_at) ?? undefined, confirmedBy: row.confirmed_by_name ?? undefined, checkIn: millis(row.check_in), checkOut: millis(row.check_out), otMinutes: row.ot_minutes ?? 0, createdAt: millis(row.client_created_at) ?? Date.now() });
