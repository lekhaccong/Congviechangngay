...
test("người ca đêm D làm thêm sau 06:00 vẫn mặc định 215%", () => {
  const result = computeOtRate("2026-09-04", "06:00", "08:00", "D");
  assert.equal(result.ratePercent, 215);
  assert.equal(result.rateLabel, "Ca đêm ngày thường 215%");
});
