...
function ctx() {
  ...
  return {
    ...
    userName: s.currentUserName || "Há»‡ thá»‘ng",
    date: s.selectedDate,
...
/** Gom cÃ¡c báº£n ghi SBD trÃ¹ng vÃ  chuyá»ƒn toÃ n bá»™ dá»¯ liá»‡u liÃªn káº¿t vá» má»™t nhÃ¢n sá»±. */
...
export async function createEmployee(data: Omit<Employee, "id" | "createdAt" | "updatedAt">) {
  ...
  if (await db.employees.where("code").equals(code).first()) throw new Error(`SBD ${code} Ä‘Ã£ tá»“n táº¡i`);
  const now = Date.now();
...
export async function updateEmployee(id: string, patch: Partial<Employee>) {
  ...
  if (!old) throw new Error("KhÃ´ng tÃ¬m tháº¥y nhÃ¢n sá»±");
  const code = normalizeEmployeeCode(patch.code ?? old.code);
  const duplicate = await db.employees.where("code").equals(code).first();
  if (duplicate && duplicate.id !== id) throw new Error(`SBD ${code} Ä‘Ã£ tá»“n táº¡i`);
...
export async function changeSchedule(employeeId: string, date: string, shiftCode: BusinessShiftCode, reason: string) {
  ...
  if (!originalShiftCode) throw new Error("NhÃ¢n sá»± chÆ°a cÃ³ lá»‹ch gá»‘c trong ngÃ y nÃ y");
  if (originalShiftCode === shiftCode) throw new Error("Ca má»›i Ä‘ang trÃ¹ng vá»›i ca thá»±c táº¿");
...
export async function swapSchedules(firstEmployeeId: string, secondEmployeeId: string, date: string, reason: string) {
  if (firstEmployeeId === secondEmployeeId) throw new Error("HÃ£y chá»n hai nhÃ¢n sá»± khÃ¡c nhau");
  const db = getDb(); const c = ctx();
  ...
  if (!firstCode || !secondCode) throw new Error("Cáº£ hai nhÃ¢n sá»± pháº£i cÃ³ lá»‹ch gá»‘c trong ngÃ y nÃ y");
  if (firstCode === secondCode) throw new Error("Hai nhÃ¢n sá»± Ä‘ang cÃ¹ng ca");
  const now = Date.now(); const batchId = nid();
...
export async function revertScheduleAdjustment(batchId: string) {
  ...
  if (!rows.some((row) => row.status === "ACTIVE")) throw new Error("Äiá»u chá»‰nh nÃ y Ä‘Ã£ Ä‘Æ°á»£c hoÃ n tÃ¡c");
...
export async function deleteGroup(id: string) {
  ...
  if (used > 0) throw new Error("NhÃ³m Ä‘ang cÃ³ nhÃ¢n sá»±, khÃ´ng thá»ƒ xÃ³a");
  const old = await db.groups.get(id);
...
export async function deleteBlock(id: string) {
  ...
  const usedByAbnormality = await db.abnormalities.filter((row) => row.workBlockId === id).count();
  if (used > 0 || usedByAbnormality > 0) throw new Error("Khá»‘i Ä‘ang cÃ³ cÃ´ng viá»‡c hoáº·c báº¥t thÆ°á»ng, khÃ´ng thá»ƒ xÃ³a");
...
export async function checkIn(employeeId: string) {
  ...
  if (!shiftId) throw new Error("ChÆ°a chá»n ca");
  const shift = await db.shifts.get(shiftId);
  if (!shift) throw new Error("KhÃ´ng tÃ¬m tháº¥y ca");
  const existing = await db.attendance
    .where("[employeeId+date+shiftId]")
    .equals([employeeId, c.date, shiftId])
    .first();
  if (existing?.checkIn) throw new Error("ÄÃ£ cháº¥m vÃ o ca nÃ y");
...
export async function checkOut(employeeId: string) {
  ...
  const existing = await db.attendance
    ...
    .equals([employeeId, c.date, shiftId])
    .first();
  if (!existing?.checkIn) throw new Error("ChÆ°a cháº¥m vÃ o");
  if (existing.checkOut) throw new Error("ÄÃ£ cháº¥m ra");
  const window = shiftWindow(c.date, shift);
  const grace = 5 * 60 * 1000;
...
export async function confirmAttendance(employeeId: string, date: string, shiftId: string, actualShiftCode: BusinessShiftCode) {
  ...
  const now = Date.now();
  const row: Attendance = existing
    ? { ...existing, status: "PRESENT", checkIn: null, checkOut: null, otMinutes: 0, note: "ÄÃ£ Ä‘áº¿n Ä‘áº§u ca", actualShiftCode, confirmedAt: now, confirmedBy: c.userName }
    : { id: nid(), employeeId, date, shiftId, checkIn: null, checkOut: null, status: "PRESENT", otMinutes: 0, note: "ÄÃ£ Ä‘áº¿n Ä‘áº§u ca", actualShiftCode, confirmedAt: now, confirmedBy: c.userName, createdAt: now };
  await db.transaction("rw", db.attendance, db.auditLogs, db.syncQueue, async () => {
    await db.attendance.put(row);
    await db.syncQueue.add(makeSyncOperation("attendance", row.id, "UPSERT", row));
...
export async function confirmAttendanceOvertime(id: string) {
  ...
  if (!old) throw new Error("KhÃ´ng tÃ¬m tháº¥y OT");
...
export async function updateTask(id: string, patch: Partial<Task>) {
  ...
  if (!old) throw new Error("KhÃ´ng tÃ¬m tháº¥y cÃ´ng viá»‡c");
...
export async function setTaskProgress(id: string, progress: number, options?: { allowOpenAbnormalities?: boolean }) {
  const db = getDb();
  const old = await db.tasks.get(id);
  if (!old) throw new Error("KhÃ´ng tÃ¬m tháº¥y cÃ´ng viá»‡c");
  if (progress >= 100 && !options?.allowOpenAbnormalities) {
    const openAbnormalities = await db.abnormalities
      .filter((row) => (row.taskId === id || (row.linkedModule === "tasks" && row.linkedId === id)) && row.status !== "CLOSED")
      .count();
    if (openAbnormalities > 0) throw new Error(`CÃ´ng viá»‡c cÃ²n ${openAbnormalities} báº¥t thÆ°á»ng chÆ°a Ä‘Ã³ng`);
  }
  const now = Date.now();
...
export async function updateAmh(id: string, patch: Partial<Amh>) {
  ...
  if (!old) throw new Error("KhÃ´ng tÃ¬m tháº¥y AMH");
...
export async function upsertLot(data: Omit<Lot, "id" | "createdAt"> & { id?: string }) {
  ...
  if (data.id) {
    ...
    if (old?.status === "CLOSED") throw new Error("Lot Ä‘Ã£ chá»‘t, khÃ´ng sá»­a trá»±c tiáº¿p");
...
export async function closeLot(lotId: string, note: string, photoId: string | null) {
  ...
  if (!lot) throw new Error("KhÃ´ng tÃ¬m tháº¥y Lot");
  if (lot.status === "CLOSED") throw new Error("Lot Ä‘Ã£ Ä‘Æ°á»£c chá»‘t");
  const c = ctx();
  const now = Date.now();
  const closure = {
    id: nid(),
...
/** Chá»‘t Invoice chá»‰ khi toÃ n bá»™ dÃ²ng HÃ ng xuáº¥t thÆ°á»ng Ä‘Ã£ hoÃ n thÃ nh. HÃ ng Air bá»‹ loáº¡i trá»«. */
export async function closeExportInvoice(invoice: string, note = "") {
  ...
  if (!exportRows.length) throw new Error("Invoice nÃ y chÆ°a cÃ³ káº¿ hoáº¡ch HÃ ng xuáº¥t thÆ°á»ng");
  const pending = exportRows.filter((row) => row.status !== "COMPLETED");
  if (pending.length) throw new Error(`CÃ²n ${pending.length}/${exportRows.length} dÃ²ng HÃ ng xuáº¥t chÆ°a hoÃ n thÃ nh`);

  const existing = (await db.lots.where("invoice").equals(normalized).toArray()).find((lot) => lot.lotCode === normalized);
  if (existing?.status === "CLOSED") throw new Error("Invoice Ä‘Ã£ Ä‘Æ°á»£c chá»‘t");
  const c = ctx();
  const now = Date.now();
