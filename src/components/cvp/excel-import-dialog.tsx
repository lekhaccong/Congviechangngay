...
import type { Employee, Shift } from "@/lib/cvp/types";
...
export function ExcelImportDialog({
  open, onClose, kind, date, shiftId, shifts = [], employees = [],
}: {
  open: boolean; onClose: () => void; kind: ExcelImportKind; date: string; shiftId: string; shifts?: Shift[]; employees?: Employee[];
}) {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (kind === "people") result = await importPeople(preview.rows as never, "", targetShiftId, preview.scheduleRows);
  ...
  const labels = kind === "people" ? ["SBD", "Họ tên", "Nhóm/Vị trí (cột D)"] : kind === "ot" ? ["SBD", "Họ tên", "Khung giờ"] : kind === "data" ? ["Mã SP", "Invoice", "Lot"] : ["Invoice", "Số kiện", "Ngày xuất"];
  const values = (row: Record<string, unknown>) => kind === "people" ? [row.code, row.name, row.groupName] : kind === "ot" ? [row.code, row.name, `${row.startTime}–${row.endTime}`] : kind === "data" ? [row.productCode, row.invoice, row.lot] : [row.invoice, row.quantity, row.exportDate];
  return <Dialog open={open} onClose={onClose} title={TITLES[kind]} wide>
    <div className="space-y-3">
      ...
      {kind === "people" ? <div>
        <p className="mb-2 text-xs text-muted">Nhóm/Vị trí được lấy tự động từ cột D của sheet “Lịch làm việc”.</p>
      ...
      <Button className="w-full" disabled={!preview?.rows.length || loading || (kind === "people" && !targetShiftId) || (kind === "ot" && !targetShiftId)} onClick={() => void save()}>{loading ? "Đang xử lý…" : `Nhập ${preview?.rows.length ?? 0} dòng`}</Button>
    </div>
  </Dialog>;
}
